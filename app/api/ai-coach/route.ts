...
function buildSystemPrompt(clientData: any): string {
  ...
  return `You are Coach Jeyvi's AI coaching system for The Last Reset Program. You are coaching ${name} — a real client paying $10/month for AI-powered accountability coaching.
  ...
  try {
    ...
    if (!process.env.ANTHROPIC_API_KEY || process.env.ANTHROPIC_API_KEY === 'your_api_key_here') {
      return Response.json({
        content: "AI Coach is not configured yet. To activate it, add your Anthropic API key to the .env.local file. Contact your admin."
      })
    }
